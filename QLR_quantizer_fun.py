...
class LearnsQuantize(InplaceFunction):
    ...
    def forward(
        cls, ctx, input, s_alpha, scale, zero_point, output_int, g, max_val, min_val, num_bits,eps, training, skew_val, BT_mode):
        ...
        zero_point = torch.tensor(int(zero_point))
...
def initial_alpha_range(input, num_bits, eps, datasetName, prop_mode, BT_mode):
    ...
    kurt_val =kurtosis(input.data.view(-1), fisher=False)
    skew_val = round(skew(input.data.view(-1)))
    ...
    for i in range(range_num):    
        ...
        find_kurt_val =kurtosis(dequant_input.data.view(-1), fisher=False)
        find_skew_val = round(skew(dequant_input.data.view(-1)))
...
class QLRQuantizer(nn.Module):
    ...
    def __init__(
        ...
        if num_bits !=32:
            ...
            self.zero_point = torch.Tensor(1).int()
            self.output_int = torch.Tensor(1).int()
            self.iter_count = 0
            self.min_val = None
            self.max_val = None
    ...
    def forward(self, input, custom_alpha = None, training=None):
        ...
        if self.custom_alpha_flag == False:
            self.inital_alpha, self.skew_val, self.kurt_val = initial_alpha_range(input, self.num_bits, self.eps, self.datasetName, self.prop_mode, self.BT_mode)

            custom_alpha.data = torch.tensor([self.inital_alpha])*1.0
        ...
        out, self.scale, self.zero_point = learns_quantize(input,custom_alpha, self.scale, self.zero_point, self.output_int, g,self.max_val,self.min_val,self.num_bits,self.eps,training, self.skew_val, self.BT_mode) 
