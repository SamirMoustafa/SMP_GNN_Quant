...
from basic_GCN_layer import MessagePassingQuant
...
class prop_part_QUANT(MessagePassingQuant):
    ...
    def __init__(self, 
                 ...
                 dataName = None,
                 qtype= False,
                 messagegroup_quantizers=None,
                 eta_lambda=None, 
                 alpha_threshold=None,finalout_quantizer=None,embedding_quant=False,
                 **kwargs):

        super(prop_part_QUANT, self).__init__(aggr='add',messagegroup_quantizers=messagegroup_quantizers,initial_graph_gamma=1.0, **kwargs)
        self.K = K
    ...
    def SMP_forward(self, x, hh, K, edge_index, inc):
        lambda_list = torch.ones([x.shape[0],1])*0.001
        phi_list = torch.ones([x.shape[0],1])*0.001
        s1_list = torch.ones([x.shape[0],1])*0.001
        s2_list = torch.ones([x.shape[0],1])*0.001
        ...
        for k in range(K):
            ...
            layer_rep_prop,_,_ = self.propagate(self.oriedge_index, x=x, edge_weight=self.edge_weight_transf, size=None, k= k, name="prop")

            sub_layer_rep = layer_rep + self.mu*eta_H*layer_rep_prop + eta_H*hh
            
            gap_rep = (sub_layer_rep - x)
            
            gap_rep_prop,_,_ = self.propagate(self.oriedge_index, x=gap_rep, edge_weight=self.edge_weight_transf, size=None, k= k, name="gap_prop")
    ...
    def EMP_forward(self, x, hh, K, edge_index, inc):
        ...
        for k in range(K):
            ...
            if lambda2 > 0:
                ...
                temp_prop_val,_,_ = self.propagate(self.oriedge_index, x=x, edge_weight=edge_weight_transf, size=None, k= k, name="prop")
